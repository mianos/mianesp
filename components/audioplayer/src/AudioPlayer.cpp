...
bool AudioPlayer::enqueue(const std::string& url, int volume) {
    return enqueue(url, std::string(), volume);
}

bool AudioPlayer::enqueue(const std::string& url, const std::string& postBody, int volume) {
    if (url.empty() || url.size() >= sizeof(PlayRequest::url)) {
        ESP_LOGW(TAG, "rejecting url (empty or too long): %u bytes", (unsigned)url.size());
        return false;
    }
    if (postBody.size() >= sizeof(PlayRequest::post_body)) {
        ESP_LOGW(TAG, "rejecting post body (too long): %u bytes", (unsigned)postBody.size());
        return false;
    }
    PlayRequest req{};
    std::strncpy(req.url, url.c_str(), sizeof(req.url) - 1);
    std::strncpy(req.post_body, postBody.c_str(), sizeof(req.post_body) - 1);
...
void AudioPlayer::playOne(const PlayRequest& req) {
    ...
    const int body_len = (int)std::strlen(req.post_body);

    ESP_LOGI(TAG, "play (vol %d, %s): %s", vol, body_len ? "POST" : "GET", req.url);
    ...
    if (body_len > 0 &&
        esp_http_client_write(http, req.post_body, body_len) != body_len) {
        ESP_LOGE(TAG, "http body write failed: %s", req.url);
        goto cleanup;
    }
