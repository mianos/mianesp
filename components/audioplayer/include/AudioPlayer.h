...
// and writes PCM to an I2S amplifier (e.g. MAX98357A, NS4168).
//
// Concurrency model: play requests are pushed onto a bounded FreeRTOS queue and
// consumed by a single dedicated task, one after another. enqueue() never
// blocks the caller (MQTT/button contexts); nothing is dropped except on queue
// overflow (logged).
//
...
//
// A request may carry a JSON POST body (e.g. for a TTS server whose speech
// endpoint is POST-only, like FastKoko's /v1/audio/speech); without one the
// URL is fetched with a plain GET.
struct AudioPlayerConfig {
    // XIAO ESP32-C3 defaults: D2=GPIO4 (data), D1=GPIO3 (bclk), D0=GPIO2 (ws).
    // M5 Atom Echo (NS4168): bclk=GPIO19, ws=GPIO33, dout=GPIO22.
...
public:
    ...
    // A non-empty postBody turns the fetch into POST with
    // Content-Type: application/json. Returns false (and logs) if the queue is
    // full or a field exceeds its fixed buffer. Non-blocking.
    bool enqueue(const std::string& url, int volume = -1);
    bool enqueue(const std::string& url, const std::string& postBody, int volume = -1);
